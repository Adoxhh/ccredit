var xmltemplateWinDetail;
var xmltemplateFormDetail;
function detailXmltemplate(){
	var record = grid.getSelectionModel().selected;
	if(record.length == 0){
		msgTishi('请选择要查看明细的项！');
		return;
	}
	initXmltemplateFormDetail();
	...
	xmltemplateWinDetail = Ext.create('Ext.Window',{
		layout:'fit',
		width:800,
		height:400,
		maximizable:true,
	...
	xmltemplateWinDetail.show();
	
	loadFormData(xmltemplateFormDetail,'../xmltemplateController/getXmltemplateById?id='+ record.items[0].data.id);
}
function initXmltemplateFormDetail(){
	xmltemplateFormDetail = Ext.create('Ext.FormPanel',{
		xtype:'form',
		waitMsgTarget:true,
