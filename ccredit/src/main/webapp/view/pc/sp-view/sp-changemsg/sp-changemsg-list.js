...
function delSpChangemsg(){
	...
	for(var i = 0; i < model.selected.length; i++){
		if(null == changemsg_id){
			changemsg_id=model.selected.items[i].data.changemsg_id;
		}else{
			changemsg_id=changemsg_id+","+model.selected.items[i].data.changemsg_id;
		}
	}
	Ext.Msg.confirm('提示','确定删除该行数据？',function(btn){
		if(btn == 'yes'){
			var params = {changemsg_id:changemsg_id};
			ajaxRequest('../spChangemsgController/delSpChangemsg',grid,params,'正在执行删除操作中！请稍后...');
		}
	});
}
/**复制一行并生成记录**/
function copySpChangemsg(){
	...
	Ext.Msg.confirm('提示','确定要复制一行并生成记录？',function(btn){
		if(btn == 'yes'){
			var params = {changemsg_id:record.items[0].data.changemsg_id};
			ajaxRequest('../spChangemsgController/copySpChangemsg',grid,params,'正在执行复制一行并生成记录！请稍后...');
		}
	});
}
/**导出**/
...
function initRight(){
	var contextmenu = new Ext.menu.Menu({
		...
		items:[{
			text:'添 加',
			glyph:0xf016,
			id:'addSpChangemsgItem',
			handler:function(){addSpChangemsg();}
		},{
			text:'编 辑',
			glyph:0xf044,
			id:'updateSpChangemsgItem',
			handler:function(){updateSpChangemsg();}
		},{
			text:'删 除',
			glyph:0xf014,
			id:'delSpChangemsgItem',
			handler:function(){delSpChangemsg();}
		},'-',{
			text:'复制一行并生成记录',
			glyph:0xf0ea,
			id:'copySpChangemsgItem',
			handler:function(){copySpChangemsg();}
		},{
			text:'明 细',
			glyph:0xf03b,
