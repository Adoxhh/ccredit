		...
		colums:[
			...
			},
			{
				...
				render:function(data, type, row, meta) {
					...
					var btn = '<button class="btn btn-default btn-sm" onclick=toLcProcessDetail("'+data+'")><i class="glyphicon glyphicon-eye-open"></i>详情</button>';
					if(lc_process_flag == 0){
			        	btn = btn +'<button class="btn btn-primary btn-sm" onclick=addLcDesign("'+data+'","'+lc_process_title+'")><i class="glyphicon glyphicon-cog"></i>设计流程</button>';
			        	btn = btn +'<button class="btn btn-success btn-sm" onclick=downFileBpmn("'+data+'")><i class="glyphicon glyphicon-save"></i>下载bpmn文件</button>';
			        	btn = btn +'<button class="btn btn-info btn-sm" onclick=downFileImg("'+data+'")><i class="glyphicon glyphicon-save"></i>下载img文件</button>';
					}else{
						btn = btn +'<button class="btn btn-info btn-sm" onclick=downFile("'+xt_attachment+'")><i class="glyphicon glyphicon-save"></i>下载附件</button>';
					}
					btn = btn +'<button class="btn btn-danger btn-sm" onclick=createDeployment("'+data+'")><i class="glyphicon glyphicon-pencil"></i>发布流程</button>';
					btn = btn +'<button class="btn btn-warnin btn-sm" onclick=showLcDeploymentHis("'+data+'","'+lc_process_title+'")><i class="glyphicon glyphicon-wrench"></i>发布历史记录</button>';
					return btn;
				}
			}
		]
