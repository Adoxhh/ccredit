package ccredit.xtmodules.xtcore.util.quartz.task;
...
import ccredit.xtmodules.xtmodel.XtMonitor;
import ccredit.xtmodules.xtmodel.XtMonitorCpu;
import ccredit.xtmodules.xtmodel.XtMonitorMem;
import ccredit.xtmodules.xtservice.XtMonitorCpuService;
import ccredit.xtmodules.xtservice.XtMonitorMemService;
import ccredit.xtmodules.xtservice.XtMonitorService;
...
public class Xt_Monitor_Task extends Thread{
	...
	public void addXtMonitor(){
        ...
        try {
			...
			xtMonitor.setXt_monitorTime(CommonUtils.getDate());
...
//				logger.info(CommonUtils.getSimpleDateFormat()+"--->主服务器信息获取成功");
	...
	public void addXtMonitorMEM(){
		...
		try {
			...
			xt_Monitor_Mem.setXt_monitor_memTime(CommonUtils.getDate());
	...
	public void addXtMonitorCPU(){
		...
		try {
	        ...
	        for(int i = 0; i < infos.length; i++) {
	            ...
	            xt_Monitor_Cpu.setXt_monitor_cpuTime(CommonUtils.getDate());
