...
import ccredit.xtmodules.xtdao.XtDataDictionaryDao;
...
public class XtDataDictionaryServiceImpl extends BaseService implements XtDataDictionaryService{
	...
	*/
	...
	/** 根据数据字典value 获取text */
	public String getDictionaryText(Map<String,String> values){
		try{
			String text  = xtDataDictionaryDao.getDictionaryText(values);
			return text;
		} catch (Exception e) {
			/**捕捉异常并回滚**/
			throw new ExceptionUtil(e.getMessage(),e.getCause());
